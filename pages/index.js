...
// This single, comprehensive component runs the entire application.
// It has been rewritten to fix all previously reported bugs.
export default function DhavaFlixApp() {
    ...
    const [modalDetails, setModalDetails] = useState(null); // Manages the 'More Info' modal
    const [videoKey, setVideoKey] = useState(null); // Manages the 'Play Trailer' modal
    ...
    useEffect(() => {
        // This effect runs only once on startup to load saved data.
        try {
    ...
    const fetchApi = useCallback(async (path, params = "") => {
        ...
        try {
            ...
            console.error(`Failed to fetch via proxy ${path}:`, err);
            return null;
        }
    }, []);
    ...
    const renderHomePage = useCallback(async () => {
        ...
        // Use a functional state update to get the latest `continueWatching` value
        setContinueWatching(currentCw => {
            if (currentCw.length > 0) {
                rows.unshift({ title: "Continue Watching", items: currentCw, endpoint: '' });
            }
            return currentCw;
        });
    ...
    const renderCategoryPage = useCallback(async (type) => {
        ...
        const staticCategories = type === 'movie' 
            ...
            : [{ title: "Airing Today", endpoint: "tv/airing_today" }, { title: "Top Rated", endpoint: "tv/top_rated" }];
        ...
        const genreCategories = genreData?.genres?.slice(0, 8).map(genre => ({ title: genre.name, endpoint: `discover/${type}`, params: `&with_genres=${genre.id}` })) || [];
    ...
    const handleToggleMyList = (item) => {
        setMyList(currentMl => {
            ...
            let newMl;
            if (index > -1) {
                newMl = currentMl.filter(i => i.id !== item.id);
            } else {
                newMl = [item, ...currentMl];
            }
    ...
    const handleToggleReminder = (item) => {
        setReminders(currentReminders => {
             const index = currentReminders.findIndex(r => r.id === item.id);
             let newReminders;
             if (index > -1) {
                newReminders = currentReminders.filter(r => r.id !== item.id);
             } else {
                newReminders = [{ id: item.id, title: item.title }, ...currentReminders];
             }
             saveToLocalStorage('dhavaflixReminders', newReminders);
             return newReminders;
        });
    }
    ...
    // --- RENDERED COMPONENTS ---
    // All components are defined within the main App function to keep it self-contained.
        ...
        return (
            <>
                ...
                <div className="py-8 md:py-12 space-y-8 md:space-y-12">
                     {contentData.map(row => <ContentRow key={row.title} row={row} />)}
                </div>
            </>
        )
    };
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between h-20">
                    <div className="flex items-center space-x-8">
                        <a onClick={() => updateView('home')} className="cursor-pointer" aria-label="DhavaFlix Home">
                        ...
                        <nav className="hidden md:flex space-x-6 text-sm font-medium">
                            {[{v:'home', t:'Home'}, {v:'tv', t:'TV Shows'}, {v:'movies', t:'Movies'}, {v:'comingsoon', t:'Coming Soon'}, {v:'mylist', t:'My List'}].map(({v, t}) => 
                                <a key={v} onClick={() => updateView(v)} className={`nav-link themed-text transition-transform duration-300 hover:scale-105 cursor-pointer ${currentView === v ? 'active' : ''}`}>{t}</a>
                            )}
                        </nav>
                    </div>
                    <div className="flex items-center space-x-4">
                        <button onClick={toggleTheme} className="themed-text p-2 rounded-full hover:bg-gray-700" aria-label="Toggle Theme">{theme === 'light' ? '☀️' : '🌙'}</button>
                        <button onClick={() => setIsSearchOpen(true)} className="themed-text" aria-label="Search"><svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg></button>
                        <a onClick={() => updateView('profile')} className="hidden md:block cursor-pointer" aria-label="Profile"><img src="https://placehold.co/40x40/0072F5/FFFFFF?text=D" alt="Profile" className="w-8 h-8 rounded-md"/></a>
                        <div className="md:hidden"><button onClick={() => setIsMobileMenuOpen(o => !o)} className="themed-text" aria-label="Open Menu"><svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16m-7 6h7" /></svg></button></div>
                    </div>
                </div>
            </div>
        </header>
            ...
            <div className="relative z-10 flex flex-col justify-end h-full w-full max-w-7xl px-4 sm:px-6 lg:px-8 pb-10 md:pb-0 md:justify-center">
                <div className="max-w-xl">
                    ...
                    <div className="mt-6 flex space-x-4">
                        <button onClick={() => handlePlayTrailer({id: item.id, type: item.media_type || (item.title ? 'movie' : 'tv')})} className="bg-white text-black font-semibold py-2 px-5 rounded flex items-center hover:bg-gray-200 transition duration-300 hover:scale-105">Play Trailer</button>
            ...
            <div className="carousel-category">
                ...
                <div className="carousel-container relative">
                    <div ref={rowRef} className="carousel-wrapper flex overflow-x-auto pb-4 scrollbar-hide px-4 sm:px-6 lg:px-8 space-x-4">
                        {row.items.map(item => item.poster_path && <ItemCard key={item.id} item={item} />)}
                    </div>
    ...
    const ItemCard = ({ item }) => {
        ...
        if (currentView === 'comingsoon') {
            ...
            return (
                <div className="group">
                    <div onClick={() => handleShowDetails(item.id, 'movie')} className="movie-card w-full aspect-[2/3] themed-bg rounded-lg overflow-hidden relative cursor-pointer">
                        <img src={`${imageBaseUrl}w500${item.poster_path}`} className="w-full h-full object-cover" loading="lazy" alt={item.title} />
                    </div>
                    <div className="flex items-center justify-between mt-2"><p className="font-semibold truncate text-sm">{item.title}</p>
                        <button onClick={() => handleToggleReminder(item)} className={`reminder-bell p-1 ${isInReminders ? 'active' : ''}`} aria-label="Set Reminder">
        ...
        return (
            <div className="group flex-shrink-0 w-36 sm:w-48 md:w-56 relative">
                <div className="movie-card w-full aspect-[2/3] themed-bg rounded-lg overflow-hidden relative">
                    <img src={`${imageBaseUrl}w500${item.poster_path}`} alt={fullItem.title} className="w-full h-full object-cover" loading="lazy" />
                </div>
                <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-lg flex flex-col items-center justify-center p-2 text-center z-20">
                    ...
                    <button onClick={() => handlePlayTrailer(fullItem)} className="w-full bg-white text-black text-sm font-semibold py-2 rounded mb-2 transition hover:scale-105">▶ Play Trailer</button>
                    <button onClick={() => handleShowDetails(fullItem.id, fullItem.type)} className="w-full bg-gray-700/80 text-sm font-semibold py-2 rounded transition hover:scale-105">ℹ More Info</button>
                </div>
            </div>
        );
    };
                        ...
                        {modalDetails === 'loading' ? <div className="p-8 text-center shimmer h-96 rounded-lg"></div> : (
                            <>
                                <div className="relative"><div className="bg-black h-48 sm:h-64 md:h-80">
                                    <img src={`${imageBaseUrl}w1280${details.backdrop_path}`} className="w-full h-full object-cover object-top opacity-50"/></div>
                                    <div className="absolute inset-0 bg-gradient-to-t from-midnight-bg via-midnight-bg/80 to-transparent"></div>
                                    <button onClick={closeModal} className="absolute top-4 right-4 bg-black/50 rounded-full w-8 h-8 flex items-center justify-center text-2xl leading-none z-10" aria-label="Close modal">&times;</button>
                                ...
                                <div className="p-4 md:p-8">
                                    ...
                                    <button onClick={() => handleToggleMyList({id: details.id, type, poster_path: details.poster_path, title: details.title || details.name})} className="bg-gray-700/80 font-semibold py-2 px-4 rounded w-full my-4 hover:bg-gray-600/70 transition">
                                        {isInList ? '✓ Added to My List' : '+ Add to My List'}
                                    </button>
                                    <p className="text-sm md:text-base">{details.overview}</p>
                                    {details.credits?.cast?.length > 0 && <div className="mt-6"><h3 className="text-xl font-bold mb-4">Cast</h3><div className="grid grid-cols-3 sm:grid-cols-6 gap-4">{details.credits.cast.slice(0, 6).map(person => person.profile_path && <div key={person.id} className="text-center"><img src={`${imageBaseUrl}w185${person.profile_path}`} className="rounded-full w-20 h-20 object-cover mx-auto" loading="lazy" alt={person.name}/><p className="text-sm mt-2">{person.name}</p></div>)}</div></div>}
    ...
    return (
        <>
            ...
            <div className="themed-bg">
                <Header />
                <MobileMenu />
                <main className="pt-20"><MainContent /></main>
                <Footer />
                <BottomNav />
                <SearchOverlay />
                <DetailsModal />
...
// --- HELPER COMPONENTS (defined outside the main component for clarity) ---
...
function ProfilePage() {
    // In a real app, this would get state via props or context.
    // For this single-file structure, we'll re-declare and load from localStorage.
    ...
    useEffect(() => {
        ...
        setRem(JSON.parse(localStorage.getItem('dhavaflixReminders')) || []);
    }, []);
    ...
    return (
        <div className="px-4 sm:px-6 lg:px-8 pt-8 min-h-screen">
            ...
            <ProfileSection title="My List" items={ml} onClear={() => clearList('dhavaflixMyList', setMl)} emptyText="Your list is empty." />
             <div className="mb-10"><h2 className="text-2xl font-semibold mb-4">Reminders</h2>
                <ul className="space-y-3 max-w-md">
                    {rem.length > 0 ? rem.map(r => <li key={r.id} className="reminder-item themed-bg p-3 rounded-lg">{r.title}</li>) : <p className="themed-text">No reminders set.</p>}
                </ul>
            </div>
        </div>
    );
    ...
    return (
        <div className="mb-10">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">
                {items.length > 0 
                    ? items.map(item => <img key={item.id} src={`https://image.tmdb.org/t/p/w500${item.poster_path}`} alt={item.title} className="w-full aspect-[2/3] themed-bg rounded-lg object-cover" />) 
...
function MobileMenu({ isOpen, updateView, currentView }) {
    // This is an example of passing state down. The state itself (`isMobileMenuOpen`) lives in the main component.
    return (
        <div className={`fixed top-0 left-0 h-full w-64 z-40 transform md:hidden ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <nav className="mt-24 flex flex-col space-y-6 px-6">
                {[{v:'home', t:'Home'}, {v:'tv', t:'TV Shows'}, {v:'movies', t:'Movies'}, {v:'comingsoon', t:'Coming Soon'}, {v:'mylist', t:'My List'}, {v:'profile', t:'Profile'}].map(({v,t}) =>
                    <a key={v} onClick={() => updateView(v)} className={`nav-link themed-text text-lg cursor-pointer ${currentView === v ? 'active' : ''} ${v === 'profile' ? 'mt-4 border-t border-gray-700 pt-4' : ''}`}>{t}</a>
                )}
            </nav>
        </div>
    );
...
function BottomNav({ updateView, currentView }) {
    return (
         <div className="fixed bottom-0 left-0 right-0 themed-bg bottom-nav flex justify-around py-2 md:hidden z-50 border-t border-gray-700/50">
            {[{v:'home', i:'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6', t:'Home'}, {v:'tv', i:'M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z', t:'TV'}, {v:'movies', i:'M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z', t:'Movies'}, {v:'mylist', i:'M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z', t:'My List'}].map(({v,i,t}) =>
                <button key={v} onClick={() => updateView(v)} className={`flex flex-col items-center text-xs themed-text ${currentView === v ? 'active' : ''}`}>
             ...
             <div className="container mx-auto px-4 pt-24">
                 ...
                 <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 mt-8 max-h-[70vh] overflow-y-auto scrollbar-hide">
                    {results.map(item => {
                        ...
                        const fullItem = {...item, type: itemType, title: item.title || item.name };
                        return (
                            <div key={item.id} className="group flex-shrink-0 relative">
                                <img src={`https://image.tmdb.org/t/p/w500${item.poster_path}`} alt={fullItem.title} className="w-full aspect-[2/3] themed-bg rounded-lg object-cover" loading="lazy" />
                                <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-lg flex flex-col items-center justify-center p-2 text-center z-20">
                                    <h3 className="font-bold text-sm mb-3">{fullItem.title}</h3>
                                    <button onClick={() => onTrailerClick(fullItem)} className="w-full bg-white text-black text-sm font-semibold py-2 rounded mb-2 transition hover:scale-105">▶ Play Trailer</button>
                                    <button onClick={() => onItemClick(fullItem.id, fullItem.type)} className="w-full bg-gray-700/80 text-sm font-semibold py-2 rounded transition hover:scale-105">ℹ More Info</button>
                                </div>
                            </div>
                        )
                    })}
