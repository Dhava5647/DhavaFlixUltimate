...
export default function TvDetailsPage() {
    ...
    if (!details) {
        return <div className="themed-bg min-h-screen text-center pt-40">Failed to load show details.</div>;
    }
                    ...
                    <div className="md:flex md:space-x-8">
                        ...
                        <div className="pt-8 md:pt-0 text-center md:text-left">
                            ...
                            <div className="flex justify-center md:justify-start flex-wrap items-center gap-x-4 gap-y-2 my-4 text-gray-300">
                                <span>{details.first_air_date?.substring(0, 4)} - {details.in_production ? 'Present' : details.last_air_date?.substring(0, 4)}</span>
                                <span>•</span>
                    ...
                    {recommendations.length > 0 && (
                        <div className="mt-12">
                            ...
                            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">
                                {recommendations.slice(0, 12).map(show => show.poster_path && (
                                    <Link key={show.id} href={`/tv/${show.id}`}>
                                        <a className="group">
                                            <Image src={`${IMAGE_BASE_URL}w500${show.poster_path}`} width={500} height={750} className="rounded-lg group-hover:scale-105 transition-transform duration-300" alt={show.name}/>
                                        </a>
                                    </Link>
                                ))}
                            </div>
