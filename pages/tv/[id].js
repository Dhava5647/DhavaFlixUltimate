            ...
            {isPlayerOpen && (
                <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-black/90 backdrop-blur-sm animate-fade-in p-4">
                    <div className="w-full max-w-6xl">
                        ...
                        <div className="aspect-video w-full"><iframe src={`https://player.autoembed.cc/embed/tv/${details.id}?s=${selectedSeason}&e=${selectedEpisode}`} title={`Watch ${details.name}`} frameBorder="0" allowFullScreen className="w-full h-full rounded-lg shadow-2xl bg-black"></iframe></div>
                    </div>
                </div>
            )}
